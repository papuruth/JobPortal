...
class Card extends React.Component {
    ...
    if (isLoggedIn() && appliedjobs) {
      appliedjobs.map((job) => {
        data.map((item, innerIndex) => {
          if (job.jobDetails.company === item.company
  ...
  fetchMoreData = () => {
    ...
    if (
      pagerData.currentPage === pagerData.pages - 1
      || !pagerData.pages
    ) {
      this.setState({
        hasMore: false,
      ...
      <div className="row">
        <div className="col-sm-12 content">
          <ul className="searchlist">
            {
              isLoggedIn() && data.length === 0 && this.user.role === 1 && <h1 style={{ textAlign: 'center' }}>No jobs posted yet. Please post some jobs!</h1>
            }
            {
              isLoggedIn() && data.length === 0 && this.user.role === 2 && <h1 style={{ textAlign: 'center' }}>No jobs posted yet. Please wait for companies!</h1>
            }
            {
              isLoggedIn() && data.length === 0 && this.user.role === 0 && <h1 style={{ textAlign: 'center' }}>No jobs posted yet. Please wait for companies or post some jobs!</h1>
            }
            {
              !isLoggedIn() && data.length === 0 && <h1 style={{ textAlign: 'center' }}>No jobs posted yet. Please wait for companies!</h1>
            }
            <InfiniteScroll
            ...
            >
              {
                data.length > 0 && data.map((job, index) => {
                  const src = config.firebase_url.concat(`${job.imageURL}?alt=media`);
                  return (
                    <li className="active1" key={'index'.concat(index)}>
                      <div className="row">
